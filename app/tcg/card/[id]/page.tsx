...
export default function TCGCardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#dfe5c9] text-[#17362c]">
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-5 py-8 md:px-8 lg:grid-cols-[minmax(280px,430px)_1fr] lg:items-start">
        {loading ? <div className="mx-auto aspect-[2.5/3.5] w-full max-w-[390px] animate-pulse rounded-2xl border-2 border-[#71816f] bg-[#f7f2d8]" /> : error ? <div className="rounded-xl border-2 border-dashed border-[#71816f] bg-[#f7f2d8] p-8"><h2 className="text-2xl font-black">Carta não encontrada.</h2><Link href="/tcg" className="mt-5 inline-block rounded border-2 border-[#17362c] bg-[#102d23] px-4 py-3 font-mono text-[9px] font-black uppercase text-white">Voltar ao catálogo</Link></div> : <div className="mx-auto w-full max-w-[390px]" style={{ perspective: "1200px" }}><div onMouseMove={moveCard} onMouseLeave={() => setRotation({ x: 0, y: 0 })} className="relative overflow-hidden rounded-[20px] border-4 border-[#f7f2d8] bg-white shadow-[12px_16px_0_rgba(23,54,44,.28),0_25px_60px_rgba(0,0,0,.25)]" style={{ transform: `rotateX(${rotation.x}deg) rotateY(${rotation.y}deg)`, transition: "transform 120ms ease-out" }}><img src={card.image} alt={card.name} className="block w-full" /><div className="pointer-events-none absolute inset-0 mix-blend-screen opacity-35" style={{ background: "linear-gradient(125deg, transparent 25%, rgba(255,255,255,.55) 43%, rgba(0,220,255,.22) 50%, rgba(255,0,100,.2) 58%, transparent 72%)" }} /></div><p className="mt-3 text-center font-mono text-[8px] font-black uppercase tracking-widest text-[#758078]">Mova o mouse para interagir</p></div>}

        {!loading && !error && <div className="rounded-2xl border-2 border-[#71816f] bg-[#f7f2d8] p-5 shadow-[6px_6px_0_rgba(23,54,44,.16)] md:p-7">
