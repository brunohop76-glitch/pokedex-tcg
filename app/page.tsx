...
export default function Home() {
  ...
  return (
    <main className="min-h-screen">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6">
          <a href="/" className="group flex items-center gap-3" aria-label="Ir para o início">
            ...
            <div>
              ...
              <p className="text-[10px] font-semibold uppercase tracking-[0.25em]">Nexus Database</p>
            </div>
          </a>
          ...
          <nav className="hidden items-center gap-8 text-xs font-bold uppercase tracking-widest md:flex" aria-label="Navegação principal">
            <a href="/">Pokédex</a>
            <a href="#generations">Gerações</a>
            <a href="#pokemon">Database</a>
            <a href="#tcg">TCG</a>
          </nav>
      ...
      <section id="generations" className="relative z-10 border-b border-white/10 py-16">
        <div className="mx-auto max-w-7xl px-6">
          <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-400">Pokémon Region Select</p>
              <h3 className="mt-2 text-3xl font-black tracking-tight md:text-5xl">Gerações <span className="text-red-500">Pokémon.</span></h3>
            </div>
            <p className="max-w-xl text-sm leading-6">Navegue pelas gerações e explore a coleção completa disponível na base de dados.</p>
          </div>
            ...
            {generations.map((generation) => {
              ...
              return (
                <button key={generation.id} type="button" onClick={() => setSelectedGeneration(generation.id)} aria-pressed={active} className={`group relative overflow-hidden rounded-2xl border p-5 text-left transition ${active ? "border-red-500/60 bg-red-500/10 shadow-[0_0_30px_rgba(239,68,68,0.08)]" : "border-white/10 bg-white/[0.02] hover:border-emerald-500/30 hover:bg-emerald-500/[0.03]"}`}>
                  ...
                  <p className="mt-3 text-lg font-black">{generation.name}</p>
                  <p className="mt-1 text-xs">{generation.region}</p>
      ...
      <section id="pokemon" className="relative z-10 py-16">
        <div className="mx-auto max-w-7xl px-6">
          <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-cyan-400">Database / {selected.region}</p>
              <h3 className="mt-2 text-3xl font-black tracking-tight md:text-5xl">Pokémon <span className="text-red-500">catalogados.</span></h3>
            </div>
          ...
          {loading ? (
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-6">
              {Array.from({ length: 12 }, (_, index) => <div key={index} className="h-56 animate-pulse rounded-2xl border border-white/10 bg-white/[0.03]" />)}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-6">
              {paginatedPokemon.map((item) => {
                ...
                return (
                  <article key={id} className="group overflow-hidden rounded-2xl border p-4 transition">
                    <div className="flex items-center justify-between">
            ...
            <div className="rounded-2xl border border-dashed border-[#71816f] bg-[#f7f2d8] p-12 text-center">
              <p className="font-mono text-lg font-black text-[#17362c]">Nenhum Pokémon encontrado.</p>
              <p className="mt-2 text-sm text-[#687366]">Tente outro nome, número ou limpe a pesquisa.</p>
            </div>
          )}

          {!loading && (
            <div className="mt-10 flex items-center justify-center gap-2">
              <button type="button" onClick={() => changePage(currentPage - 1)} disabled={currentPage === 1} className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 disabled:opacity-30">Anterior</button>
              <button type="button" onClick={() => changePage(currentPage + 1)} disabled={currentPage === totalPages} className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2 text-[10px] font-black uppercase tracking-widest text-zinc-400 disabled:opacity-30">Próxima</button>
            </div>
          )}
        </div>
      </section>

      <section id="tcg" className="relative z-10 py-16">
        <div className="mx-auto max-w-7xl px-6">
          <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.04] to-transparent p-8 md:p-12">
            ...
            <div className="mt-3 flex flex-col justify-between gap-8 md:flex-row md:items-end">
              <div>
                <h3 className="text-3xl font-black tracking-tight md:text-5xl">TCG <span className="text-red-500">Database.</span></h3>
