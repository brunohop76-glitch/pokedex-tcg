    ...
    <main className="min-h-screen bg-[#dfe5c9] text-[#17362c]">
      ...
      <div className="mx-auto max-w-7xl px-6 py-12 md:py-16">
        ...
        <section>
          ...
          {loadingCards ? <div className="rounded-2xl border border-[#71816f] bg-[#f7f2d8] p-8 text-sm">Consultando cartas TCG...</div> : filteredCards.length ? <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6">{filteredCards.map((card) => <article key={card.id} className="group overflow-hidden rounded-2xl border border-[#71816f]/60 bg-[#f7f2d8] p-3 transition hover:-translate-y-1 hover:border-[#d71920] hover:shadow-lg"><div className="flex min-h-56 items-center justify-center rounded-xl bg-black/5 p-2">{card.image ? <img src={`${card.image}/high.webp`} alt={card.name ?? "Carta Pokémon"} className="max-h-56 object-contain transition group-hover:scale-[1.02]"/> : <span className="text-xs text-[#758078]">Sem imagem</span>}</div><h3 className="mt-3 truncate text-xs font-black">{card.name ?? "Carta TCG"}</h3><p className="mt-1 truncate text-[8px] uppercase tracking-wider text-[#758078]">{[card.set?.name, card.rarity].filter(Boolean).join(" • ") || "Carta Pokémon TCG"}</p></article>)}</div> : <div className="rounded-2xl border border-dashed border-[#71816f] bg-[#f7f2d8] p-8 text-sm">Nenhuma carta TCG encontrada.</div>}
        </section>
      </div>
    </main>
  );
